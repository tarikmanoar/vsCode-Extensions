...
class ObjectExplorerService {
        ...
        const handler = (result) => __awaiter(this, void 0, void 0, function* () {
            if (result.success) {
                ...
                if (self._currentNode && (self._currentNode.sessionId === result.sessionId)) {
                    ...
                    self._currentNode = treeNodeInfo_1.TreeNodeInfo.fromNodeInfo(result.rootNode, result.sessionId, undefined, self._currentNode.connectionCredentials, nodeLabel, Constants.serverLabel);
                }
                else {
                    nodeLabel = !nodeLabel ? self.createNodeLabel(nodeConnection) : nodeLabel;
                    self._currentNode = treeNodeInfo_1.TreeNodeInfo.fromNodeInfo(result.rootNode, result.sessionId, undefined, nodeConnection, nodeLabel, Constants.serverLabel);
                }
    ...
    handleExpandSessionNotification() {
        ...
        const handler = (result) => {
            if (result && result.nodes) {
                ...
                const children = result.nodes.map(node => treeNodeInfo_1.TreeNodeInfo.fromNodeInfo(node, result.sessionId, self._currentNode, credentials));
    ...
    refreshNode(node) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this._connectionManager.client.sendRequest(refreshSessionRequest_1.RefreshRequest.type, refreshParams);
