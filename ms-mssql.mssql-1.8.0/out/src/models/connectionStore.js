...
class ConnectionStore {
     ...
     */
    static formatCredentialId(server, database, user, itemType) {
    ...
    lookupPassword(connectionCredentials) {
        return __awaiter(this, void 0, void 0, function* () {
            const credentialId = ConnectionStore.formatCredentialId(connectionCredentials.server, connectionCredentials.database, connectionCredentials.user, ConnectionStore.CRED_PROFILE_USER);
        ...
        return new Promise((resolve, reject) => {
            ...
            self._context.globalState.update(Constants.configRecentConnections, configValues)
                ...
                if (conn.savePassword) {
                    self.doSavePassword(conn, interfaces_1.CredentialsQuickPickItemType.Mru);
                }
                // And resolve / reject at the end of the process
                resolve(undefined);
            }, err => {
    ...
    saveProfilePasswordIfNeeded(profile) {
        ...
        return this.doSavePassword(profile, interfaces_1.CredentialsQuickPickItemType.Profile);
    }
    doSavePassword(conn, type) {
        let self = this;
        return new Promise((resolve, reject) => {
