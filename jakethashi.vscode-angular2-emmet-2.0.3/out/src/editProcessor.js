...
class EditProcessor {
     ...
     */
    sanitizeContent(content, lineInfo) {
     ...
     */
    sanitizeAbbreviation(abbr, angularInfo) {
        abbr = abbr.trim();
     ...
     */
    findLine(search, direction, line, offsetChar) {
        ...
        if (!isMultiline) {
            ...
            if (dStart && tStart) {
                ...
                if (dStart && dEnd && dStart.line < cursorLine &&
                    ...
                    let abbrCandidate = vscode.window.activeTextEditor.document.getText(new vscode.Range(new vscode.Position(cursorLine, 0), vscode.window.activeTextEditor.selection.end));
