...
class DataSource {
    ...
    generateGitCommandFormats() {
        ...
        let dateType = config.dateType() === 0 ? '%at' : '%ct';
    ...
    getCommits(repo, branches, maxCommits, showRemoteBranches, remotes, hideRemotes) {
        const config = config_1.getConfig();
        return Promise.all([
            ...
            let commits = results[0], refData = results[1], stashes = results[2], i, unsavedChanges = null;
                ...
                for (i = 0; i < commits.length; i++) {
                    if (refData.head === commits[i].hash) {
                        ...
                        if (unsavedChanges !== null) {
                            commits.unshift({ hash: utils_1.UNCOMMITTED, parents: [refData.head], author: '*', email: '', date: Math.round((new Date()).getTime() / 1000), message: 'Uncommitted Changes (' + unsavedChanges.changes + ')' });
                        }
                        break;
                    }
                }
    ...
    getCommitDetails(repo, commitHash, baseHash) {
        return Promise.all([
            this.spawnGit(['show', '--quiet', commitHash, '--format=' + this.gitFormatCommitDetails], repo, (stdout) => {
                ...
                if (!err) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (inSubmoduleSection && (match = lines[i].match(pathProp)) !== null) {
                            let root = yield this.repoRoot(utils_1.getPathFromUri(vscode_1.Uri.file(path.join(repo, utils_1.getPathFromStr(match[1])))));
    ...
    cherrypickCommit(repo, commitHash, parentIndex, noCommit) {
    ...
    popStash(repo, selector) {
        return this.runGitCommand(['stash', 'pop', selector], repo);
    }
    saveStash(repo, message, includeUntracked) {
        if (this.gitExecutable === null) {
            return Promise.resolve(utils_1.UNABLE_TO_FIND_GIT_MSG);
        }
        if (utils_1.compareVersions(this.gitExecutable, '2.13.2') < 0) {
    ...
    getLog(repo, branches, num, includeTags, includeRemotes, order, remotes, hideRemotes) {
