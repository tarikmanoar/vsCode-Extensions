...
class GitGraphView {
    constructor(extensionPath, dataSource, extensionState, avatarManager, repoManager, logger, loadRepo, column) {
        ...
        this.panel.iconPath = config_1.getConfig().tabIconColourTheme() === 0
        ...
        this.panel.webview.onDidReceiveMessage((msg) => __awaiter(this, void 0, void 0, function* () {
            ...
            switch (msg.command) {
                ...
                case 'applyStash':
                    this.sendMessage({
                        command: 'applyStash',
                        error: yield this.dataSource.applyStash(msg.repo, msg.selector)
                    });
                    break;
                ...
                case 'commitDetails':
                    let data = yield Promise.all([
                        msg.commitHash !== utils_1.UNCOMMITTED ? this.dataSource.getCommitDetails(msg.repo, msg.commitHash, msg.baseHash) : this.dataSource.getUncommittedDetails(msg.repo),
                ...
                case 'getSettings':
                    ...
                    break;
                case 'loadCommits':
                    this.sendMessage(Object.assign(Object.assign({ command: 'loadCommits' }, yield this.dataSource.getCommits(msg.repo, msg.branches, msg.maxCommits, msg.showRemoteBranches, msg.remotes, msg.hideRemotes)), { hard: msg.hard }));
                    break;
                case 'loadRepoInfo':
                    ...
                    this.sendMessage(Object.assign(Object.assign({ command: 'loadRepoInfo' }, repoInfo), { hard: msg.hard, isRepo: isRepo }));
                ...
                case 'popStash':
                    this.sendMessage({
                        command: 'popStash',
                        error: yield this.dataSource.popStash(msg.repo, msg.selector)
                    });
                    break;
                case 'pruneRemote':
                    this.sendMessage({
                ...
                case 'saveStash':
                    this.sendMessage({
                        command: 'saveStash',
                        error: yield this.dataSource.saveStash(msg.repo, msg.message, msg.includeUntracked)
                    });
                    break;
        ...
        if (this.dataSource.isGitExecutableUnknown()) {
			...
			<div id="view">
				<div id="controls">
					...
					<label id="showRemoteBranchesControl"><input type="checkbox" id="showRemoteBranchesCheckbox"><span class="customCheckbox"></span>Show Remote Branches</label>
