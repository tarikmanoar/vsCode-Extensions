...
class TreeNodeInfo extends vscode.TreeItem {
    ...
    static fromNodeInfo(nodeInfo, sessionId, parentNode, connectionCredentials, label) {
        const treeNodeInfo = new TreeNodeInfo(label ? label : nodeInfo.label, nodeInfo.nodeType, nodeInfo.isLeaf ? vscode.TreeItemCollapsibleState.None :
            (nodeInfo.nodeType === Constants.serverLabel ? vscode.TreeItemCollapsibleState.Expanded :
                vscode.TreeItemCollapsibleState.Collapsed), nodeInfo.nodePath, nodeInfo.nodeStatus, nodeInfo.nodeType, sessionId, connectionCredentials, parentNode, nodeInfo.metadata);
        return treeNodeInfo;
    }
    /** Getters */
    get nodePath() {
