...
class DataSource {
    ...
    generateGitCommandFormats() {
        ...
        this.gitFormatCommitDetails = [
            '%H', '%P',
            useMailmap ? '%aN' : '%an', useMailmap ? '%aE' : '%ae', dateType, useMailmap ? '%cN' : '%cn',
    ...
    getCommits(repo, branches, maxCommits, showRemoteBranches, showTags, remotes, hideRemotes) {
        const config = config_1.getConfig();
        return Promise.all([
    ...
    getUncommittedDetails(repo) {
        return Promise.all([
            ...
            return {
                commitDetails: {
                    hash: utils_1.UNCOMMITTED, parents: [], author: '', email: '', date: 0, committer: '', signature: null, body: '',
    ...
    merge(repo, obj, actionOn, createNewCommit, squash) {
        return __awaiter(this, void 0, void 0, function* () {
            let args = ['merge', obj];
            if (squash)
                args.push('--squash');
